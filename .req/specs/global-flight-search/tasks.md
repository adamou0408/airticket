...
- Plan: .req/specs/global-flight-search/plan.md
...
### 任務 D.1：每日排程爬取引擎 [depends: B.1]
- **對應 User Story**：US-15（真實票價資料）
- **描述**：建立排程系統，每天凌晨自動爬取預設熱門航線和使用者自訂航線
- **驗收條件**：
  - [ ] 預設熱門航線：TPE↔NRT、TPE↔KIX、TPE↔ICN、TPE↔BKK、TPE↔SIN 等
  - [ ] 每天凌晨 3:00 自動執行
  - [ ] 爬取結果存入快取
  - [ ] 執行紀錄可查詢（成功/失敗/航線）
- **測試策略**：整合測試 — 手動觸發排程、驗證快取寫入
- **狀態**：`todo`

### 任務 D.2：使用者自訂排程 API [depends: D.1]
- **對應 User Story**：US-15
- **描述**：使用者可新增/刪除自己關注的航線，系統每天自動爬取
- **驗收條件**：
  - [ ] POST `/api/crawl-schedules` 新增自訂排程（origin, destination）
  - [ ] GET `/api/crawl-schedules` 列出我的排程
  - [ ] DELETE `/api/crawl-schedules/{id}` 刪除排程
  - [ ] DB 持久化（crawl_schedules table）
  - [ ] 每日排程包含所有使用者的自訂航線（去重）
- **測試策略**：整合測試 — CRUD API、去重邏輯
- **狀態**：`todo`

### 任務 D.3：排程管理前端頁面 [depends: D.2]
- **對應 User Story**：US-15
- **描述**：前端頁面讓使用者管理自己的爬取排程
- **驗收條件**：
  - [ ] 「我的關注航線」列表
  - [ ] 一鍵新增：搜尋完機票後可「加入每日追蹤」
  - [ ] 可刪除不需要的排程
