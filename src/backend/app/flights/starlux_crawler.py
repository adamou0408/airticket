"""Starlux Airlines (星宇航空) flight crawler.

Crawls starlux-airlines.com for flight prices.
...
class StarluxCrawler(FlightCrawler):
    """Crawls Starlux Airlines (JX) website for flight prices.

    Strategy:
    1. Open Starlux booking page with Playwright
    2. Fill search parameters via URL or form
    3. Wait for results
    4. Parse flight data from DOM
    """
    ...
    async def search(
        ...
        if not HAS_PLAYWRIGHT:
            logger.warning("Playwright not installed, skipping Starlux crawler")
            return []

        try:
            return await self._crawl(origin, destination, travel_date, passengers)
        except Exception as e:
            logger.error(f"Starlux crawler failed: {e}")
            return []
    ...
    async def _crawl(
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Starlux booking URL with parameters
                date_str = travel_date.strftime('%Y-%m-%d')
                url = (
                    f"https://www.starlux-airlines.com/zh-TW/booking/search"
                    f"?tripType=OW&origin={origin}&destination={destination}"
                    f"&departureDate={date_str}&adults={passengers}&children=0&infants=0"
                )

                logger.info(f"Starlux: searching {origin}→{destination} on {date_str}")
                ...
                flight_cards = await page.query_selector_all(
                    ...
                    '[class*="result"], [class*="itinerary"], .flight-option'
                )
                ...
                if not results:
                    logger.info("Starlux: no results from DOM, trying text extraction")
        ...
        logger.info(f"Starlux: found {len(results)} flights")
        return results
    ...
    def _parse_flight_card(
        ...
        flight_nums = re.findall(r'(JX\d{2,4})', text)
        ...
        if len(times) >= 2 and prices:
            ...
            return FlightResult(
                airline="星宇航空",
                flight_number=flight_nums[0] if flight_nums else "JX---",
                origin=origin,
                destination=dest,
    ...
    def _extract_prices_from_text(
        ...
        """Fallback: extract price info from raw page text."""
        results = []
        ...
        for i, price_str in enumerate(prices[:5]):
            ...
            dep_time = times[i * 2] if i * 2 < len(times) else "09:00"
            arr_time = times[i * 2 + 1] if i * 2 + 1 < len(times) else "13:00"
            fn = flight_nums[i] if i < len(flight_nums) else f"JX{800 + i}"
